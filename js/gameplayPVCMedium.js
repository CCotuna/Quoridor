...
function setup() {
  ...
  pawn2 = new Pawn(375, 555, 30, player2.color);
  board.push(pawn2);
...
function canReachParallelSide(pawn, targetY) {
  ...
  if (!checkForWall(x, y)) {
    ...
    for (const { x, y } of nextMoves) {
      if (!checkForWall(x, y) && canReachParallelSide(pawn, targetY)) {
        return true; // Found a path to the target position
      }
    }
  }

  return false; // No path to the target position
}
